...
function initRevealAnimations() {
    const revealTargets = document.querySelectorAll('.hero__feature, .hero__latest, .news-card, .category-block, .radio__panel, .footer__inner');
...
// Header oculto al inicio del home y compacto al hacer scroll
function updateHeaderState() {
    const header = document.getElementById('main-header');
    if (!header) return;

    const hasHero = Boolean(document.querySelector('.hero'));
    const heroHanger = document.getElementById('hero-hanger');
    const scrollY = window.scrollY || window.pageYOffset || 0;
    const showHeader = !hasHero || scrollY > 56;
    ...
    if (heroHanger) {
        heroHanger.classList.toggle('is-hidden', showHeader);
    }
